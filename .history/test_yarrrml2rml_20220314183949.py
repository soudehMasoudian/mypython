...
class test_translate(unittest.TestCase):
    def test_convert_yarrrml2rml(self):
        print('ha ha')
        yarrrml = open("./tests/yarrrml-input-test.yarrrml")
        payload = {'test': yarrrml.read}
        session = requests.session
        rml_output = requests.post("http://127.0.0.1:5000/rmlconversion", files={"test": yarrrml})
        rml_output_path = open("./tests/rml-output-test.ttl", "w")
        rml_output_path.write(rml_output.__str__)
